...
def scoped_spawn_device_env(
    ...
    # Some model components are deliberately placed on a GPU owned by another
    # stage (for example a stateless vision tower colocated with Code2Wav).
    # Keep the stage's declared devices authoritative for TP/world-size and
    # append these physical devices only to the child process visibility.
    auxiliary_devices = os.environ.get("VLLM_OMNI_AUX_VISIBLE_DEVICES", "")
    if auxiliary_devices.strip():
        visible = [item.strip() for item in stage_visible_devices.split(",") if item.strip()]
