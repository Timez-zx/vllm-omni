...
"""Mailbox: persistent two-slot shared-memory transport.
...
Enabled via the factory: VLLM_OMNI_MAILBOX=1 (the default) substitutes this
class where the deploy yaml says SharedMemoryConnector.
"""
...
class ShmMailboxConnector(SharedMemoryConnector):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        logger.info("ShmMailboxConnector active (slot=%dB x2 per edge-session)", self.capacity)
