...
class OmniStreamingVideoHandler:
    ...
    def __init__(
        ...
        # Background arrival work must not enter Thinker while a foreground
        # request is crossing its prefill. The gate reopens at the first
        # foreground engine output, rather than after speech generation ends.
        ...
        try:
            ...
            for admission in active:
                if admission.task.done():
                    continue
                # The non-P/D pipeline owns the complete request locally.
                # AsyncOmni's cancellation handler aborts an already admitted
                # internal request, while cancellation before admission simply
                # prevents the optional background work from entering. Do not
                # make a foreground query wait for background AddRequest.
    ...
    async def handle_session(self, websocket: WebSocket) -> None:
        ...
        try:
            ...
            async def _finish_arrival_prefill() -> None:
                ...
                The global handoff has cancelled every registered warm-up. A
                task still waiting for the one background slot is not visible
                in that table, so cancel it explicitly before foreground
                prefill can reopen the gate. The final request remains correct
                because it carries the complete canonical prompt.
                """
                nonlocal arrival_prefill_task, arrival_prefill_dirty
                started = _time.monotonic()
                arrival_prefill_dirty = False
            ...
            async def _start_query_turn(*, query_text: str) -> None:
                ...
                # Close background admission before any query-side await.
                # Registered work is cancelled and cleaned through AsyncOmni;
                # work still waiting for the background slot is cancelled
                # below.
    ...
    async def _generate_history_summary(
        ...
        *,
        ...
        The summary instruction follows the completed canonical history, so
        ordinary prefix caching can reuse that history. The request emits text
        only and therefore never enters Talker or Code2Wav.
        """
        ...
        if arrival_admission is not None:
            arrival_admission.request_id = request_id
        ...
        *,
        ...
        async def _minimum_drop_for_target(
            ...
            *,
            ...
            # as many recent turns as the target allows with O(log N) renders.
        ...
        if summary:
            # Build the candidate lineage privately. The visible session state
            # changes only after summary generation and prompt rendering both
            # succeed.
            try:
        ...
        if summary_config is None:
            # A proactive summary failure preserves history and may be retried.
            # At the hard limit, retain the old drop-complete-turns fallback so
            # request correctness never depends on successful summary decode.
            if (
    ...
    async def _process_query_engine(
        ...
        try:
            ...
            async for output in result_gen:
                # Thinker prefill has completed before the pipeline exposes its
                # first output. Allow background arrival work again without
                # holding the gate through Thinker/Talker decode.
            ...
            if not interrupted:
                ...
                _reset_thinker_lineage(config)
            # Appending a completed turn and rewriting old context are one
            # per-session transaction. This prevents an arrival summary from
            # committing drop offsets selected from stale history length.
            async with config._history_compaction_lock:
