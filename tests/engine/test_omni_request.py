...
def test_pd_prefill_payload_round_trips_to_scheduler_new_request_data():
    from vllm_omni.core.sched.output import OmniNewRequestData
    from vllm_omni.engine import OmniPDPrefillPayload

    payload = OmniPDPrefillPayload(
        prompt_layer_0=torch.ones(2, 3),
        prompt_layer_24=torch.full((2, 3), 48.0),
        prompt_token_ids=[1, 2],
    )
    request = OmniRequest(
        request_id="pd-decode",
    ...
    scheduled = OmniNewRequestData.from_request(request, ([0],))

    assert scheduled.pd_prefill_payload is payload
