...
class User:
    ...
    def __init__(self, uid: int, rep: int, frames: list[str], turns: int, seed: int,
        ...
        if opts is not None and opts.content == "synthetic":
            ...
            # MU_UNIQUE_INPUTS=1: additionally no frame repeats WITHIN a
            # session (320 > ~31 frames/turn-window x 10 turns), so the mm
            # encoder cache cannot dedupe across turns -- the default 16-frame
            # cycle lets every re-sampled frame hit the encoder cache, which
            # understates vision-encode cost vs real video.
            n_pool = 320 if os.environ.get("MU_UNIQUE_INPUTS", "") == "1" else 16
            frames = [base64.b64encode(synth_frame_jpeg(f"{self.name} {i}")).decode()
                      for i in range(n_pool)]
        self.frames = frames
        self._audio_turn_seq = 0
        self.turns = turns
    ...
    async def _stream_audio_input(self, ws) -> None:
        ...
        if os.environ.get("MU_UNIQUE_INPUTS", "") == "1":
            # Distinct bytes per (user, turn): the default deterministic tone is
            # byte-identical engine-wide, so the audio encoder runs ONCE for the
            # whole bench and every later turn hits the mm cache.
            import numpy as np
            seed_v = (hash(self.name) ^ (self._audio_turn_seq * 0x9E3779B1)) & 0xFFFF
            self._audio_turn_seq += 1
            t = np.arange(int(16000 * self.audio_input_s)) / 16000.0
            f1, f2 = 200 + seed_v % 120, 430 + (seed_v >> 4) % 160
            env = 0.35 * (1.0 + np.sin(2 * np.pi * 3.0 * t)) / 2.0
            wave = env * (0.6 * np.sin(2 * np.pi * f1 * t) + 0.4 * np.sin(2 * np.pi * f2 * t))
