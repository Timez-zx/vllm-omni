...
## WP7 帧信箱:帧编码按拍合批(2026-08-13,已实现 + 验证)

**问题**:`prefill_frames_on_arrival` 是到达即触发——u24×v160 下 150 帧/s 各自
相位散落;API 服务进程没有任何时钟,帧的模板渲染 + process_inputs 随机打断
serving 事件循环。引擎侧 T 的 tick 环虽能把 80ms 窗内的到达聚进同一 pass,
但服务端的到达散是上游源头。

**实现**(`VLLM_OMNI_TEMPORAL_FRAME_TICK=1`,独立开关,T 族待遇):帧到达进
信箱(复用 frame_buffer——它本来就是被拒帧的候车室),每 session 一个定时器
睡到下一个**全局 80ms 拍沿**,经 msg_queue 注入 `_internal.frame_flush`
(单写者不破),整箱帧合**一个 chunk** 追加。守卫与软失败契约原样:turn 忙
不动、帧永不丢。相位对齐 = 全体 session 对同一 CLOCK_MONOTONIC 按 80ms 取整
——与引擎 pacer 同一网格公式,**无需共享注册表**。stage2 同时补了
[SCHED-STEP] 证据线(VLLM_OMNI_LOG_SCHED_STEPS),声码合批从此可量测。

**验证**(T 臂全 FP8,v160×u24×8 轮 mixed,真实到达 0-40s,去 2 轮热身
n=144/臂):

| | FT 关 | FT 开 |
|---|---|---|
| 开口 p50 (ms) | 552 | **381(-31%)** |
| 开口 p99 (ms) | 1,244 | 1,265(持平) |
| 迟到块 | 0.61% | 0.56% |
| 每轮卡住 (ms) | 21 | 23 |
| 质量 | cps 14.7,短答地板 47 | cps 15.2,地板同 47 |

**诚实归因**:引擎侧合批两臂相近(stage0 预填批 p50 3 vs 2、≥2 请求占约
60%——T 的 tick 环已在聚;ChunkTickGate 声码批 p50=2、≥2 占 56%,两臂同)。
帧信箱的 31% 开口收益来自**服务端**:追加只在拍沿发生,查询到达时 7/8 的
时间窗是净空的,turn 开口不再排在单帧追加后面;turn 忙期间帧安静躺箱、由
turn chunk 顺路带走(8 帧整箱冲刷 96 次,零事故)。

**建议**:帧密负载(≤160ms 取帧)随 T 臂开启;480ms 标准负载未测,预期收益
按帧密度折减。
