...
**终局(休息区修复 + 到达分布判决)**:探针定位残余离群 = park 唤醒后请求滞留
skipped_waiting(状态已 WAITING 但人在"休息区",调度器扫到看运气,实测 15-token
段干等 12s)。修复:唤醒即重新入队(omni_ar_scheduler,~10 行,下游分支早有
同款动作,stage0 路径漏了)。修复后同步开局下集体波变胖(4→10→14 慢轮——
休息区慢发现曾是意外错峰器,本研究第三个"bug 即泄压阀");判决实验:开局摊到
0-40s(真实到达分布,MU_STAGGER_S),**p99 972ms、max 1001ms、>1.5s 零轮、
miss 0.43%(全程最低)**——残余尾巴 100% 为压测同秒开局的人为产物。
链路终局:64.5s → 3.0s → p95 1.16s → **p99 0.97s(真实到达)**,66×,质量无损。

**出厂配置认证(2026-08-13,用户选型:不带图压缩)**:提前预热(0.6×触发建、
原线换)被实测否决——parked 影子占引擎槽,32 影子+32 活请求 > mns56,全场
集体变慢(p50 2.8s,107/128 慢轮),第四个"泄压阀"教训:影子短命本身在给
槽位泄压;不带图种子 ~200 token 现建现换 1-3s,提前无利可图,已回退。
同步开局下休息区修复的代价如实:12s 无界抽签 → 2-9s 有界排队(慢轮 4-8→10-14),
真实流量下无此波。**终版认证(不带图+全修复+真实到达,long u32):
p50 428 / p95 621 / p99 774 / max 778ms,零慢轮零异常,15.1 字/秒。
自基线 p99 64.5s 起 83×。**
