#!/usr/bin/env bash
# S_high ALONE -- the patch sanity check, re-run after it was lost.
#
# The first attempt produced nothing: a stale chain fired when phase 1 was
# stopped for an edit, ran a detail probe that died on an argparse error, and
# that probe's closing kill_all took down the server this arm was using. The
# arm is re-run alone so the four substantive arms do not have to be repeated.
#
#   run_fix_latency_S.sh [reps] [sessions]      default: 60 4
#
...
LOG=$RES/server_fxS.log
EVENTS=$RES/stage0_events_fxS.jsonl
...
start_server() {   # cfg label
  ...
  PA_STAGE0_PROBE=1 PA_STAGE0_PROBE_OUT=$RES/stage0_probe_fxS.json \
