...
def check(result_dir: Path) -> int:
    ...
    if summary.get("workload_schema") != 4:
        print("   !! workload schema is not 4; this result does not use the fixed audio-ready-500 measurement contract")
        bad += 1
    ...
    if len(finite_ids) != expected_requests or len(set(finite_ids)) != len(finite_ids):
        print(
            f"   !! finite requests: lines={len(finite_ids)} unique={len(set(finite_ids))} expected={expected_requests}"
        )
        bad += 1
    else:
    ...
    final_frame_counts = [int(value) for value in re.findall(r"\[finite-request\].* frames=(\d+)", log)]
    ...
    prefix_hits = [int(value) for value in re.findall(r"\[prefix-cache\].*hit_tokens=(\d+)", log)]
    if expected_requests > int(summary.get("users") or 0) and not any(prefix_hits):
        ...
        print(f"   ok prefix-cache observations: {nonzero}/{len(prefix_hits)} hit, max={max(prefix_hits)} tokens")
